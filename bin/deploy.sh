...
DATE=$(date +%Y%m%d%H%M)
# 基础路径
BASE_PATH=/work/projects/metass-server
# 编译后 jar 的地址。部署时，Jenkins 会上传 jar 包到该目录下
SOURCE_PATH=$BASE_PATH/metass-server/target
# 服务名称。同时约定部署服务的 jar 包名字也为它。
SERVER_NAME=metass-server
# 环境
PROFILES_ACTIVE=prod
# 健康检查 URL
HEALTH_CHECK_URL=http://127.0.0.1:48080/actuator/health/
# 日志存放
LOG_PATH=/root/meta-server.log
...
function start() {
    ...
    # 开始启动
    BUILD_ID=dontKillMe nohup java -server $JAVA_OPS $JAVA_AGENT -jar $BASE_PATH/$SERVER_NAME.jar --spring.profiles.active=$PROFILES_ACTIVE > $LOG_PATH 2>&1 &
