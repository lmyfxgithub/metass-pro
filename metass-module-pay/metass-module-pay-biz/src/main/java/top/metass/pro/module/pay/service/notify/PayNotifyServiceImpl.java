...
import top.metass.pro.framework.common.util.date.DateUtils;
...
import top.metass.pro.module.pay.dal.dataobject.notify.PayNotifyLogDO;
import top.metass.pro.module.pay.dal.dataobject.notify.PayNotifyTaskDO;
...
import top.metass.pro.module.pay.dal.mysql.notify.PayNotifyLogMapper;
import top.metass.pro.module.pay.dal.mysql.notify.PayNotifyTaskMapper;
import top.metass.pro.module.pay.dal.redis.notify.PayNotifyLockRedisDAO;
import top.metass.pro.module.pay.enums.notify.PayNotifyStatusEnum;
...
import top.metass.pro.module.pay.api.notify.dto.PayOrderNotifyReqDTO;
import top.metass.pro.module.pay.api.notify.dto.PayRefundNotifyReqDTO;
import top.metass.pro.module.pay.service.order.PayOrderExtensionService;
...
import static top.metass.pro.framework.common.util.date.LocalDateTimeUtils.*;
import static top.metass.pro.module.pay.framework.job.config.PayJobConfiguration.NOTIFY_THREAD_POOL_TASK_EXECUTOR;
...
public class PayNotifyServiceImpl implements PayNotifyService {
    ...
    private Integer processNotifyResult(PayNotifyTaskDO task, CommonResult<?> invokeResult, Throwable invokeException) {
        ...
        if (invokeResult != null && invokeResult.isSuccess()) {
            PayOrderDO order = orderService.getOrder(task.getDataId()); // 不进行非空判断，有问题直接异常
            PayOrderExtensionDO orderExtensionByOrderNo = orderExtensionService.getOrderExtensionByOrderNo(order.getChannelOrderNo());
            Map<String, String> channelExtras = orderExtensionByOrderNo.getChannelExtras();
            String spuId = channelExtras.get("spuId");
            String creator = orderExtensionByOrderNo.getCreator();

            memberUserApi.updateUserSpu(Long.parseLong(creator), Long.parseLong(spuId));
